...
func main() {
	...
	dsPath := flag.String("dataset", datasetPath, "path to CIFAR-10 dataset")
	...
	var (
		vaeModel     *pkg.CifarVAE
		decoderModel *pkg.CifarVAE
	)

	switch *mode {
	case "encode":
		vaeModel = pkg.CreateCifarVAETrainModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
	case "encode_interp", "mix":
		vaeModel = pkg.CreateCifarVAETrainModel(2, latentDim, device, nil)
		decoderModel = pkg.CreateCifarVAEInferenceModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
		decoderModel.Compile()
	default:
		vaeModel = pkg.CreateCifarVAEInferenceModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
	}
	...
	case "encode":
		var ds *cifar_10.Dataset
		if *useTest {
			ds, err = cifar_10.CreateTestingDataset(*dsPath)
		} else {
			ds, err = cifar_10.CreateTrainingDataset(*dsPath)
		}
		if err != nil {
			err = fmt.Errorf("cifar_10 dataset: %w", err)
			return
		}
	...
	dims := mtl.MTLSize{W: cifar_10.ImageWidth, H: cifar_10.ImageHeight, D: cifar_10.ImageDepthRGB * miniBatchSize}

	outFloats := outData.Data.GetFloats()
	outImgs, e := createRGBImage(dims, outFloats[:dims.Length()], *normalize)
	if e != nil {
		err = fmt.Errorf("createRGBImage output: %w", e)
		return
	}

	if *mode == "encode" {
		inImgs, e := createRGBImage(dims, input[:dims.Length()], false)
		if e != nil {
			err = fmt.Errorf("createRGBImage input: %w", e)
			return
		}
	...
	if *mode == "encode_interp" || *mode == "mix" {
		inDims := mtl.MTLSize{W: cifar_10.ImageWidth, H: cifar_10.ImageHeight, D: cifar_10.ImageDepthRGB * 2}
		inImgs, e := createRGBImage(inDims, input[:inDims.Length()], false)
		if e != nil {
			err = fmt.Errorf("createRGBImage input: %w", e)
			return
		}
...
func createRGBImage(dims mtl.MTLSize, data []float32, normalize bool) ([][]byte, error) {
	mh := dims.W
	mw := dims.H
	res := [][]byte{}
	...
	for imageIndex := 0; imageIndex < dims.D/3; imageIndex++ {
		base := imageIndex * ch
		imgFloats := data[base : base+ch]
...
func encodeMu(model *pkg.CifarVAE, latentDim, batch int, device *proc.Device) ([]float32, error) {
