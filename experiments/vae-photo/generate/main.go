...
func main() {
	...
	dsPath := flag.String("dataset", datasetPath, "path to photo dataset")
	flag.Parse()
	...
	var (
		vaeModel     *vaephoto.PhotoVAE
		decoderModel *vaephoto.PhotoVAE
	)

	switch *mode {
	case "encode":
		vaeModel = vaephoto.CreatePhotoVAETrainModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
	case "encode_interp", "mix":
		vaeModel = vaephoto.CreatePhotoVAETrainModel(2, latentDim, device, nil)
		decoderModel = vaephoto.CreatePhotoVAEInferenceModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
		decoderModel.Compile()
	default:
		vaeModel = vaephoto.CreatePhotoVAEInferenceModel(miniBatchSize, latentDim, device, nil)
		vaeModel.Compile()
	}
	...
	case "encode":
		ds, e := vaephoto.LoadPhotoDataset(*dsPath, vaephoto.PhotoPatchSize)
		if e != nil {
			err = fmt.Errorf("LoadPhotoDataset: %w", e)
			return
		}
	...
	dims := mtl.MTLSize{W: vaephoto.PhotoPatchSize, H: vaephoto.PhotoPatchSize, D: vaephoto.ImageDepthRGB * miniBatchSize}
	...
	if *mode == "encode_interp" || *mode == "mix" {
		inDims := mtl.MTLSize{W: vaephoto.PhotoPatchSize, H: vaephoto.PhotoPatchSize, D: vaephoto.ImageDepthRGB * 2}
...
func encodeMu(model *vaephoto.PhotoVAE, latentDim, batch int, device *proc.Device) ([]float32, error) {
