...
func main() {
	...
	cfg.WeightsProvider = &gpt2.WeightsProvider{
		WeightsSTReader: gpt2.WeightsSTReader{WeightsReader: weightsReader},
	}

	m := gpt2.NewModel(cfg, device, nil)
	m.Compile()
	m.LoadFromProvider()
	...
	allTokens := make([]uint32, 0, cfg.ContextLength+*steps)
	allTokens = append(allTokens, inputTokens...)

	zeroFloats := make([]float32, cfg.ContextLength)
	for i := range zeroFloats {
		zeroFloats[i] = 50256
	}
	for i := 0; i < *steps; i++ {
		...
		allTokens = append(allTokens, uint32(next))
	...
	fmt.Println("")
	decoded, err := tokenizer.Decode(allTokens)
	if err != nil {
