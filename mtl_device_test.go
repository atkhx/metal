...
func TestMTLDeviceProperties(t *testing.T) {
	device := MustMTLCreateSystemDefaultDevice()
	defer device.Release()

	require.NotNil(t, device.GetDeviceID())
...
func TestMTLDeviceMethods(t *testing.T) {
	device := MustMTLCreateSystemDefaultDevice()
	require.NotNil(t, device.GetDeviceID())
	defer device.Release()

	commandQueue, err := device.NewCommandQueue()
